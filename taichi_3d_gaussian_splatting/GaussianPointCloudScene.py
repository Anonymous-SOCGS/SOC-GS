...
class GaussianPointCloudScene(torch.nn.Module):
    ...
    class PointCloudSceneConfig(YAMLWizard):
        # 4 quat + 3 sacle + 1 alpha + 16R-SH + 16G-SH + 16B-SH + 16 MS-SH = R72 features
        # num_of_features: int = 56
        num_of_features: int = 88
        max_num_points_ratio: Optional[float] = 10.0
        add_sphere: bool = True
        sphere_radius_factor: float = 4.0
        num_points_sphere: int = 10000
        max_initial_covariance: Optional[float] = 3000.0
        initial_alpha: float = -0.0
        initial_covariance_ratio: float = 0.1
    ...
    def __init__(
        ...
        # expand the num of points from num_points to max_num_points_ratio(10) * num_points
    ...
    def initialize(self, point_cloud_rgb: Optional[torch.Tensor] = None):
        with torch.no_grad():
            ...
            self.point_cloud_features[:, 56] = 1.0
            self.point_cloud_features[:, 57:72] = 0.0
            self.point_cloud_features[:, 72] = 1.0
            self.point_cloud_features[:, 73:88] = 0.0
            if point_cloud_rgb is not None:
                ...
                self.point_cloud_features[(self.point_invalid_mask == 0), 56] = \
                    self._logit(point_cloud_rgb[:, 3]) / c0
                self.point_cloud_features[(self.point_invalid_mask == 0), 72] = \
                    self._logit(point_cloud_rgb[:, 4]) / c0
    ...
    def to_parquet(self, path: str):
        ...
        feature_columns = [f"cov_q{i}" for i in range(4)] + \
            ...
            [f"ms_sh{i}" for i in range(16)] + \
            [f"ir_sh{i}" for i in range(16)]
    ...
    def from_trained_parquet(path: str, config=PointCloudSceneConfig()):
        ...
        # scene_df.loc[:, 'ms'] = ms_initial

        # equal to point features
        feature_columns = [f"cov_q{i}" for i in range(4)] + \
                          [f"cov_s{i}" for i in range(3)] + \
                          [f"alpha{i}" for i in range(1)] + \
                          [f"r_sh{i}" for i in range(16)] + \
        ...
        df_has_color = "r" in scene_df.columns and "g" in scene_df.columns \
                       and "b" in scene_df.columns and "ms" in scene_df.columns and "ir" in scene_df.columns
        ...
        if not set(feature_columns).issubset(set(scene_df.columns)):
            ...
            point_cloud_rgb_ms = scene_df[["r", "g", "b", "ms", "ir"]
            ].to_numpy() if df_has_color else None
            scene.initialize(point_cloud_rgb=point_cloud_rgb_ms)
    ...
    def from_parquet(path: str, config=PointCloudSceneConfig()):
        # load parquet file and set (r+g+b)//3 as initial ms color
        scene_df = pd.read_parquet(path)
        color_initial = (scene_df['r'] + scene_df['g'] + scene_df['b']) // 3
        scene_df.loc[:, 'ms'] = color_initial
        scene_df.loc[:, 'ir'] = color_initial
        ...
        if not set(feature_columns).issubset(set(scene_df.columns)):
            ...
            point_cloud_rgb_ms_ir = scene_df[["r", "g", "b", "ms", "ir"]
                                       ].to_numpy() if df_has_color else None
            scene.initialize(point_cloud_rgb=point_cloud_rgb_ms_ir)
    ...
    def _add_sphere(scene_df: pd.DataFrame, radius_factor: float, num_points: int):
        ...
        df_has_color = "r" in scene_df.columns and "g" in scene_df.columns \
                       and "b" in scene_df.columns and "ms" in scene_df.columns \
                       and "ir" in scene_df.columns

        # calculate the radius of SH
        ...
        if df_has_color:
            ...
            ms = np.ones((num_points, 1)) * (255 // 2)
            ir = np.ones((num_points, 1)) * (255 // 2)
            points = np.concatenate([points, rgb, ms, ir], axis=1)
            columns += ["r", "g", "b", "ms", "ir"]
